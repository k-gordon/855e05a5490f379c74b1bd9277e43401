...
function emitGlobalEvent(payload) {
  if (typeof window.onGlobalEvent === 'function') {
    window.onGlobalEvent(JSON.stringify(payload));
  } else {
    console.warn('Dropped event before Flutter listener was ready: ' + payload.name);
  }
}

function emitRegisteredEvent(payload) {
  if (typeof window.onRegisteredEvent === 'function') {
    window.onRegisteredEvent(JSON.stringify(payload));
  }
}
...
export function msgbox(type, title, text, link) {
  ...
  emitGlobalEvent({ name: 'msgbox', type, title, text, link: link ?? '', hasRetry });
}
...
function loadRecentPeers() {
  ...
  if (peersRecent) {
    emitRegisteredEvent({ name: 'load_recent_peers', peers: JSON.stringify(peersRecent) });
  }
}
...
function loadFavPeers() {
  try {
    ...
    if (peersFav) {
      emitRegisteredEvent({ name: 'load_fav_peers', peers: JSON.stringify(peersFav) });
    }
  } catch (e) {
